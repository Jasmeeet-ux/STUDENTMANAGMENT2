...
$preselect_batch_id   = (int)($_GET['batch_id'] ?? 0);
$preselect_batch_name = '';
if ($preselect_batch_id) {
    $stmtPre = $pdo->prepare("SELECT batch_name FROM batches WHERE id = ?");
    $stmtPre->execute([$preselect_batch_id]);
    $preselect_batch_name = $stmtPre->fetchColumn() ?? '';
}
?>
<!DOCTYPE html>
<html lang="en">
...
.cal-cell .status-dot{position:absolute;bottom:5px;left:50%;transform:translateX(-50%);width:7px;height:7px;border-radius:50%;pointer-events:none;}
.cal-cell.P .status-dot{background:#22c55e;}
.cal-cell.A .status-dot{background:#ef4444;}
.cal-cell.Lv .status-dot{background:#f59e0b;}
.cal-cell.L .status-dot{background:#f97316;}
.cal-cell.O .status-dot{background:#e879f9;}
...
.leg-dot.P{background:#22c55e;}
.leg-dot.A{background:#ef4444;}
.leg-dot.Lv{background:#f59e0b;}
.leg-dot.L{background:#f97316;}
.leg-dot.O{background:#e879f9;}
...
<div class="sidebar">
  ...
  <a href="attendance.php" class="active"><span class="sb-icon">✅</span>Mark Attendance</a>
        ...
        <select id="batchSelect" onchange="loadStudents()">
            ...
            <?php foreach ($batches as $b): ?>
                <option value="<?= htmlspecialchars($b['batch_name']) ?>"
                    <?= ($b['batch_name'] === $preselect_batch_name) ? 'selected' : '' ?>>
                    <?= htmlspecialchars($b['batch_name']) ?>
                </option>
            <?php endforeach; ?>
        </select>
        <span class="batch-info" id="batchInfo">Select a batch to begin</span>
    </div>
    ...
    <div class="content-row">
        <div class="students-panel">
            ...
            <div class="students-scroll" id="studentsList">
                <div class="empty-students"><div class="e-icon">👥</div><p>Select a batch first</p></div>
            </div>
        </div>
...
const preselect='<?= htmlspecialchars($preselect_batch_name, ENT_QUOTES) ?>';
if(preselect){ window.addEventListener('DOMContentLoaded',()=>{ document.getElementById('batchSelect').value=preselect; loadStudents(); }); }

function loadStudents(){
    activeBatch=document.getElementById('batchSelect').value;
    if(!activeBatch) return;
    fetch(`attendance.php?get_students=1&batch=${encodeURIComponent(activeBatch)}`)
        .then(r=>r.json()).then(data=>{
            document.getElementById('stuCount').textContent=data.length;
            document.getElementById('batchInfo').textContent=data.length+' students · Batch '+activeBatch;
...
function loadBadges(reg_no){
    fetch(`attendance.php?get_attendance=1&reg_no=${encodeURIComponent(reg_no)}`)
        .then(r=>r.json()).then(data=>{
...
function saveStatus(status){
    ...
    fetch('attendance.php',{method:'POST',body:fd}).then(r=>r.text())
