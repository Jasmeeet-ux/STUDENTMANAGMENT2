...
// ── Add Multiple Topics (subparts) at once ──
if (isset($_POST['add_topics'])) {
    ...
    header("Location: course_modules.php?course_id=$course_id&added=1"); exit;
}
...
if (isset($_POST['edit_topic'])) {
    ...
    header("Location: course_modules.php?course_id=$course_id&edited=1"); exit;
}
...
if (isset($_GET['del_topic'])) {
    ...
    header("Location: course_modules.php?course_id=$course_id"); exit;
}
...
if (isset($_POST['add_mcq'])) {
    ...
    header("Location: course_modules.php?course_id=$course_id&mcq=1"); exit;
}
...
if (isset($_POST['upload_assignment'])) {
    ...
    header("Location: course_modules.php?course_id=$course_id&ass=1"); exit;
}

// ── Delete MCQ ──
if (isset($_GET['del_mcq'])) {
    $pdo->prepare("DELETE FROM topic_mcq WHERE id=? AND course_id=?")->execute([(int)$_GET['del_mcq'],$course_id]);
    header("Location: course_modules.php?course_id=$course_id&tab=mcq"); exit;
}
...
if (isset($_POST['edit_mcq'])) {
    ...
    $correct = trim($_POST['correct_ans']);
    $pdo->prepare("UPDATE topic_mcq SET question=?,option_a=?,option_b=?,option_c=?,option_d=?,correct_ans=? WHERE id=? AND course_id=?")
        ->execute([$q,$oa,$ob,$oc,$od,$correct,$id,$course_id]);
    header("Location: course_modules.php?course_id=$course_id&tab=mcq&edited_mcq=1"); exit;
}

// ── Delete Assignment ──
if (isset($_GET['del_ass'])) {
    $pdo->prepare("DELETE FROM topic_assignments WHERE id=? AND course_id=?")->execute([(int)$_GET['del_ass'],$course_id]);
    header("Location: course_modules.php?course_id=$course_id&tab=assignment"); exit;
}

// ── Edit Assignment ──
if (isset($_POST['edit_ass'])) {
    $id    = (int)$_POST['ass_id'];
    $title = trim($_POST['ass_title']);
    ...
    $pdo->prepare("UPDATE topic_assignments SET title=?,instructions=?,file_path=? WHERE id=? AND course_id=?")
        ->execute([$title,$inst,$file_path,$id,$course_id]);
    header("Location: course_modules.php?course_id=$course_id&tab=assignment&edited_ass=1"); exit;
}
...
<head>
<meta charset="UTF-8">
<title>Modules — <?= h($course['course_name']) ?></title>
...
.subpart-fields input::placeholder{color:#cbd5e1;}
...
.btn-add-sub{width:100%;padding:10px;background:#f0fdf4;color:#16a34a;border:2px dashed #86efac;border-radius:9px;font-size:13px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all 0.15s;margin-bottom:12px;}
.btn-add-sub:hover{background:#dcfce7;border-color:#4ade80;}
...
<div class="main">
  <div class="page-header">
    <div>
      <h1>🗂️ <?= h($course['course_name']) ?></h1>
      <p>Manage modules, subparts, MCQs and assignments</p>
    </div>
    <a href="courses.php" class="btn-back">← Courses</a>
  </div>

  <?php if(isset($_GET['added'])): ?><div class="alert alert-ok">✅ Subparts added successfully!</div><?php endif; ?>
  <?php if(isset($_GET['edited'])): ?><div class="alert alert-ok">✅ Subpart updated!</div><?php endif; ?>
  <?php if(isset($_GET['edited_mcq'])): ?><div class="alert alert-ok">✅ MCQ updated!</div><?php endif; ?>
  <?php if(isset($_GET['edited_ass'])): ?><div class="alert alert-ok">✅ Assignment updated!</div><?php endif; ?>
  ...
  <div class="tabs">
    <a href="?course_id=<?= $course_id ?>&tab=modules"    class="tab-btn <?= $tab==='modules'?'active':'' ?>">🗂️ Modules & Subparts</a>
    <a href="?course_id=<?= $course_id ?>&tab=mcq"        class="tab-btn <?= $tab==='mcq'?'active':'' ?>">🧠 MCQ</a>
    <a href="?course_id=<?= $course_id ?>&tab=assignment" class="tab-btn <?= $tab==='assignment'?'active':'' ?>">📁 Assignment</a>
  </div>
  ...
  <div class="layout">
    ...
    <div class="form-card">
      ...
      <form method="POST" id="addForm">
        ...
        <div class="field">
          <label>Module Name (Heading)</label>
          <input type="text" name="module_name"
            placeholder="e.g. SEO – Search Engine Optimization"
            list="mod-list" autocomplete="off" required>
          <datalist id="mod-list">
            <?php foreach(array_keys($modules) as $mn): ?>
              <option value="<?= h($mn) ?>">
            <?php endforeach; ?>
          </datalist>
        </div>

        <div class="subparts-label">
          <span>Subparts</span>
          <span class="sub-count-badge" id="subCountBadge">1 subpart</span>
        </div>
        ...
        <div class="subparts-list" id="subpartsList">
          <div class="subpart-row" id="row-1">
            ...
            <div class="subpart-fields">
              <input type="text" name="topics[]" placeholder="Subpart name e.g. On-Page SEO" required>
        ...
        <button type="button" class="btn-add-sub" onclick="addRow()">
          ➕ Add Another Subpart
        </button>

        <button type="submit" name="add_topics" class="btn-submit">
          💾 Save Module & Subparts
        </button>
      </form>
    </div>
    ...
    <div>
      <?php if($modules): ?>
        <?php foreach($modules as $mod_name => $topics): ?>
        <div class="module-block">
          <div class="module-head">
            <h3>📂 <?= h($mod_name) ?></h3>
            <div class="module-head-right">
              <span class="mod-count"><?= count($topics) ?> subparts</span>
              <a href="?course_id=<?= $course_id ?>&del_module=<?= urlencode($mod_name) ?>"
                 class="btn-del-mod"
                 onclick="return confirm('Delete entire module?')">🗑️ Delete Module</a>
            </div>
          </div>
          ...
          <div class="topic-row">
            ...
            <div class="topic-actions">
              <button class="btn-edit-t"
                onclick="openEdit(<?= $t['id'] ?>,'<?= h($mod_name) ?>','<?= h($t['topic_name']) ?>','<?= h($t['video_url']??'') ?>',<?= (int)($t['topic_order']??0) ?>)">✏️ Edit</button>
              <a href="?course_id=<?= $course_id ?>&del_topic=<?= $t['id'] ?>"
                 class="btn-del-t"
                 onclick="return confirm('Delete this subpart?')">🗑️</a>
            </div>
          </div>
          <?php endforeach; ?>
        </div>
        <?php endforeach; ?>
      <?php else: ?>
        <div class="no-modules">📭 No modules yet. Add your first module from the form.</div>
      <?php endif; ?>
    </div>
  </div>
  ...
  <div class="layout">
    ...
    <div>
      <?php if($modules): foreach($modules as $mod=>$topics): foreach($topics as $t):
          ...
          $tq->execute([$course_id,$t['topic_name']]);
          $tqs=$tq->fetchAll(PDO::FETCH_ASSOC);
          if(!$tqs) continue; ?>
        <div class="module-block">
          <div class="module-head">
            <h3>🧠 <?= h($t['topic_name']) ?></h3>
            <span class="mod-count"><?= count($tqs) ?> questions</span>
          </div>
          ...
          <div class="topic-row">
            ...
            <button class="btn-edit-t" style="margin-right:4px;"
              onclick="openMcqEdit(<?= $q['id'] ?>,'<?= addslashes(h($q['question'])) ?>','<?= addslashes(h($q['option_a'])) ?>','<?= addslashes(h($q['option_b'])) ?>','<?= addslashes(h($q['option_c']??'')) ?>','<?= addslashes(h($q['option_d']??'')) ?>','<?= $q['correct_ans'] ?>')">✏️</button>
            <a href="?course_id=<?= $course_id ?>&tab=mcq&del_mcq=<?= $q['id'] ?>"
               class="btn-del-t" onclick="return confirm('Delete MCQ?')">🗑️</a>
          </div>
          <?php endforeach; ?>
        </div>
        ...
        <div class="topic-row">
          ...
          <div class="topic-info">
            ...
            <div style="display:flex;gap:4px;align-items:center;">
              <button class="btn-edit-t"
                onclick="openAssEdit(<?= $a['id'] ?>,'<?= addslashes(h($a['title'])) ?>','<?= addslashes(h($a['instructions']??'')) ?>','<?= addslashes(h($a['file_path']??'')) ?>')">✏️</button>
              <a href="?course_id=<?= $course_id ?>&tab=assignment&del_ass=<?= $a['id'] ?>"
                 class="btn-del-t" onclick="return confirm('Delete this assignment?')">🗑️</a>
            </div>
        </div>
        <?php endforeach; ?>
      </div>
...
function addRow() {
  rowCount++;
  const list = document.getElementById('subpartsList');
  const div  = document.createElement('div');
  div.className = 'subpart-row';
  div.id = 'row-' + rowCount;
  div.innerHTML = `
    <div class="subpart-num">${rowCount}</div>
    <div class="subpart-fields">
      <input type="text" name="topics[]" placeholder="Subpart name" required>
      <input type="url"  name="videos[]" placeholder="Video URL (optional)">
    </div>
    <button type="button" class="btn-remove-sub" onclick="removeRow(${rowCount})">×</button>
  `;
...
function renumber() {
  document.querySelectorAll('#subpartsList .subpart-row').forEach((row, i) => {
    row.querySelector('.subpart-num').textContent = i + 1;
  });
  updateBadge();
}

function updateBadge() {
  const n = document.querySelectorAll('#subpartsList .subpart-row').length;
  document.getElementById('subCountBadge').textContent = n + ' subpart' + (n > 1 ? 's' : '');
}

function openEdit(id, mod, topic, video, order) {
  document.getElementById('edit_id').value    = id;
  document.getElementById('edit_mod').value   = mod;
...
function setMcqModule(sel) {
  document.getElementById('mcq_module_hidden').value = sel.options[sel.selectedIndex].dataset.module || '';
}
function setAssModule(sel) {
  document.getElementById('ass_module_hidden').value = sel.options[sel.selectedIndex].dataset.module || '';
}

function openMcqEdit(id, q, oa, ob, oc, od, correct) {
  document.getElementById('mcq_edit_id').value       = id;
  document.getElementById('mcq_edit_q').value        = q;
  document.getElementById('mcq_edit_oa').value       = oa;
  document.getElementById('mcq_edit_ob').value       = ob;
  document.getElementById('mcq_edit_oc').value       = oc;
  document.getElementById('mcq_edit_od').value       = od;
  document.getElementById('mcq_edit_correct').value  = correct;
  document.getElementById('mcqEditOverlay').classList.add('on');
}
function closeMcqEdit() { document.getElementById('mcqEditOverlay').classList.remove('on'); }
document.getElementById('mcqEditOverlay').addEventListener('click', function(e) { if(e.target===this) closeMcqEdit(); });

function openAssEdit(id, title, inst, file) {
  document.getElementById('ass_edit_id').value       = id;
  document.getElementById('ass_edit_title').value    = title;
  document.getElementById('ass_edit_inst').value     = inst;
  document.getElementById('ass_edit_file').value     = file;
  document.getElementById('assEditOverlay').classList.add('on');
}
function closeAssEdit() { document.getElementById('assEditOverlay').classList.remove('on'); }
document.getElementById('assEditOverlay').addEventListener('click', function(e) { if(e.target===this) closeAssEdit(); });
</script>

<!-- MCQ Edit Modal -->
<div class="overlay" id="mcqEditOverlay">
  <div class="modal">
    <h3>✏️ Edit MCQ</h3>
    <form method="POST">
      <input type="hidden" name="mcq_id" id="mcq_edit_id">
      <div class="field"><label>Question</label><textarea name="question" id="mcq_edit_q" required style="height:70px;resize:vertical;"></textarea></div>
      <div class="field"><label>Option A</label><input type="text" name="option_a" id="mcq_edit_oa" required></div>
      <div class="field"><label>Option B</label><input type="text" name="option_b" id="mcq_edit_ob" required></div>
      <div class="field"><label>Option C</label><input type="text" name="option_c" id="mcq_edit_oc"></div>
      <div class="field"><label>Option D</label><input type="text" name="option_d" id="mcq_edit_od"></div>
      <div class="field">
        <label>Correct Answer</label>
        <select name="correct_ans" id="mcq_edit_correct" required>
          <option value="A">A</option><option value="B">B</option>
          <option value="C">C</option><option value="D">D</option>
        </select>
      </div>
      <div class="modal-btns">
        <button type="submit" name="edit_mcq" class="btn-save-edit">💾 Save</button>
        <button type="button" class="btn-close-modal" onclick="closeMcqEdit()">Cancel</button>
      </div>
    </form>
  </div>
</div>
...
<div class="overlay" id="assEditOverlay">
  <div class="modal">
    <h3>✏️ Edit Assignment</h3>
    <form method="POST" enctype="multipart/form-data">
      <input type="hidden" name="ass_id" id="ass_edit_id">
      <input type="hidden" name="existing_file" id="ass_edit_file">
      <div class="field"><label>Title</label><input type="text" name="ass_title" id="ass_edit_title" required></div>
      <div class="field"><label>Instructions</label><textarea name="instructions" id="ass_edit_inst" style="height:70px;resize:vertical;"></textarea></div>
      <div class="field"><label>Replace File (optional)</label><input type="file" name="assignment_file" accept=".pdf,.doc,.docx,.ppt,.pptx"></div>
      <div class="modal-btns">
        <button type="submit" name="edit_ass" class="btn-save-edit">💾 Save</button>
        <button type="button" class="btn-close-modal" onclick="closeAssEdit()">Cancel</button>
      </div>
    </form>
  </div>
</div>
