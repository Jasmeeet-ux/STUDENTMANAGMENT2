...
$pre_batch_id = (int)($_GET['batch_id'] ?? 0);
...
$batches_stmt = $pdo->prepare("SELECT id,batch_name,timing_start,timing_end FROM batches WHERE course_id=? ORDER BY batch_name ASC");
$batches_stmt->execute([$course_id]);
$batches = $batches_stmt->fetchAll(PDO::FETCH_ASSOC);

// Topics from modules (suggestions)
$topics_stmt = $pdo->prepare("SELECT DISTINCT topic_name FROM course_modules WHERE course_id=? ORDER BY topic_order ASC");
$topics_stmt->execute([$course_id]);
$topics_list = array_column($topics_stmt->fetchAll(PDO::FETCH_ASSOC),'topic_name');

// Already used session topics
...
if (isset($_POST['edit_session'])) {
    ...
    $sess_type = trim($_POST['session_type'] ?? 'lecture');
    $status    = trim($_POST['status'] ?? 'scheduled');
    if ($sid && $date && $topic) {
        $pdo->prepare("UPDATE class_sessions SET date=?,start_time=?,end_time=?,topic=?,session_type=?,status=? WHERE id=? AND course_id=?")
            ->execute([$date,$start?:null,$end?:null,$topic,$sess_type,$status,$sid,$course_id]);
    }
    header("Location: course_sessions.php?course_id=$course_id&edited=1"); exit;
}
...
if (isset($_POST['add_session'])) {
    ...
    foreach ($batches as $b) {
        if ($b['id'] == $batch_id) { $batchName = $b['batch_name']; break; }
    }

    if ($batch_id && $date && $topic) {
        $pdo->prepare("INSERT INTO class_sessions (course_id,batch_id,batch_no,date,start_time,end_time,topic,status,session_type) VALUES (?,?,?,?,?,?,?,?,?)")
            ->execute([$course_id,$batch_id,$batchName,$date,$start?:null,$end?:null,$topic,'scheduled',$sess_type]);
    }
    header("Location: course_sessions.php?course_id=$course_id&added=1"); exit;
}

// ── Status Change ──
if (isset($_GET['action'],$_GET['sid'])) {
    $sid=(int)$_GET['sid'];
    $map=['complete'=>'completed','cancel'=>'cancelled','reschedule'=>'scheduled'];
    $ns=$map[$_GET['action']]??null;
    if($ns) $pdo->prepare("UPDATE class_sessions SET status=? WHERE id=?")->execute([$ns,$sid]);
    header("Location: course_sessions.php?course_id=$course_id"); exit;
}

// ── Delete ──
if (isset($_GET['delete'])) {
    $pdo->prepare("DELETE FROM class_sessions WHERE id=? AND course_id=?")->execute([(int)$_GET['delete'],$course_id]);
    header("Location: course_sessions.php?course_id=$course_id"); exit;
}

// ── Fetch Sessions ──
$fb = $_GET['batch'] ?? '';
$q = "SELECT cs.*, b.batch_name as b_name FROM class_sessions cs LEFT JOIN batches b ON b.id=cs.batch_id WHERE cs.course_id=?";
$p = [$course_id];
if ($fb) { $q.=" AND cs.batch_id=?"; $p[]=(int)$fb; }
$q.=" ORDER BY cs.date DESC, cs.start_time DESC";
$sstmt=$pdo->prepare($q); $sstmt->execute($p);
$sessions=$sstmt->fetchAll(PDO::FETCH_ASSOC);

// ✅ Batch timing map for JS
$batch_timings = [];
foreach ($batches as $b) {
    $batch_timings[$b['id']] = [
...
// Pre-selected batch timing
$pre_batch_name  = '';
$pre_batch_start = '';
$pre_batch_end   = '';
foreach ($batches as $b) {
    if ($b['id'] == $pre_batch_id) {
        $pre_batch_name  = $b['batch_name'];
        $pre_batch_start = $b['timing_start'] ?? '';
        $pre_batch_end   = $b['timing_end']   ?? '';
        break;
    }
}
...
<head>
<meta charset="UTF-8">
<title>Sessions — <?= h($course['course_name']) ?></title>
...
.fb{padding:6px 14px;border-radius:7px;font-size:12.5px;font-weight:700;text-decoration:none;background:#2563eb;color:#fff;transition:opacity 0.15s;}
...
.bc{background:#dcfce7;color:#15803d;}.bca{background:#fee2e2;color:#dc2626;}.bcr{background:#f3f4f6;color:#374151;}.bcd{background:#fef3c7;color:#92400e;}.bce{background:#eff6ff;color:#2563eb;border:none;cursor:pointer;font-family:'DM Sans',sans-serif;}
...
<div class="main">
  <div class="page-header">
    <div>
      <h1>📅 Sessions — <?= h($course['course_name']) ?></h1>
      <p>Add and manage sessions batch-wise</p>
    </div>
    <a href="course_modules.php?course_id=<?= $course_id ?>" class="btn-back">← Modules</a>
  </div>
  ...
  <div class="layout">
    ...
    <div class="form-card">
      ...
      <form method="POST">
        ...
        <?php if($pre_batch_id): ?>
          <!-- view_batch se aaya — batch fixed -->
          <input type="hidden" name="batch_id" value="<?= $pre_batch_id ?>">
          <div class="field">
            <label>Batch</label>
        ...
        <?php else: ?>
          <!-- Normal batch dropdown with auto-timing -->
          <div class="field">
            <label>Batch</label>
            <select name="batch_id" id="batchSelect" required onchange="fetchTiming(this.value)">
              <option value="" disabled selected>Select batch</option>
              <?php foreach($batches as $b): ?>
                <option value="<?= $b['id'] ?>"><?= h($b['batch_name']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        <?php endif; ?>
        ...
        <div class="field">
          ...
          <datalist id="topics-datalist">
            <?php foreach($all_suggestions as $t): ?>
              <option value="<?= h($t) ?>">
            <?php endforeach; ?>
          </datalist>
        </div>
        ...
        <div class="fields-row">
          <div class="field">
            ...
            <input type="time" name="start_time" id="startTime"
              value="<?= $pre_batch_start ? date('H:i', strtotime($pre_batch_start)) : '' ?>">
          </div>
          <div class="field">
            <label>End Time</label>
            <input type="time" name="end_time" id="endTime"
              value="<?= $pre_batch_end ? date('H:i', strtotime($pre_batch_end)) : '' ?>">
          </div>
        </div>
    ...
    <div>
      <div class="filter-row">
        ...
        <?php foreach($batches as $b): ?>
          <a href="?course_id=<?= $course_id ?>&batch=<?= $b['id'] ?>" class="fb <?= $fb==$b['id']?'':'off' ?>"><?= h($b['batch_name']) ?></a>
        <?php endforeach; ?>
        <?php if($fb): ?><a href="?course_id=<?= $course_id ?>" class="fb off">✕ All</a><?php endif; ?>
      </div>
      ...
      <div class="sess-card">
        ...
        <?php if($sessions): ?>
        <table>
          <thead><tr><th>Date</th><th>Batch</th><th>Type</th><th>Topic</th><th>Time</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
          ...
          <tr>
            ...
            <td style="font-weight:600;"><?= h($s['b_name']??$s['batch_no']??'—') ?></td>
            ...
            <td>
              <div class="act-btns">
                ...
                <?php if($s['status']==='scheduled'): ?><a href="?course_id=<?= $course_id ?>&action=cancel&sid=<?= $s['id'] ?>" class="btn-sm bca">❌ Cancel</a><?php endif; ?>
                <?php if($s['status']==='cancelled'): ?><a href="?course_id=<?= $course_id ?>&action=reschedule&sid=<?= $s['id'] ?>" class="btn-sm bcr">🔄 Reschedule</a><?php endif; ?>
                <a href="?course_id=<?= $course_id ?>&delete=<?= $s['id'] ?>" class="btn-sm bcd" onclick="return confirm('Delete?')">🗑️</a>
              </div>
            </td>
          </tr>
          <?php endforeach; ?>
...
<div class="modal-overlay" id="editOverlay">
  <div class="modal">
    ...
    <form method="POST">
      ...
      <div class="field">
        <label>Topic</label>
        <input type="text" name="topic" id="editTopic" list="topics-datalist" required>
      </div>
      <div class="field">
        <label>Session Type</label>
...
<script>
const batchTimings = <?= json_encode($batch_timings) ?>;

function fetchTiming(batchId) {
    if (!batchId || !batchTimings[batchId]) return;
    const t = batchTimings[batchId];
    if (t.start) document.getElementById('startTime').value = t.start.slice(0, 5);
    if (t.end)   document.getElementById('endTime').value   = t.end.slice(0, 5);
}
