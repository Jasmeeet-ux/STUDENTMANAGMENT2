...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $pdo->prepare("
            UPDATE modules 
            SET course_id = ?, module_number = ?, title = ?, duration = ?
            WHERE id = ?
        ");
...
<div class="flex h-screen">
    ...
    <div class="flex-1 p-6">
        ...
        <h2 class="text-2xl font-semibold mb-4">Edit Module: <?= htmlspecialchars($module['title']) ?></h2>
        ...
        <form method="post" class="bg-white p-4 rounded-lg shadow space-y-4">

            <div>
                <label class="block mb-1">Course *</label>
                <select name="course_id" class="border p-2 w-full">
                    <?php foreach ($courses as $c): ?>
                        <option value="<?= $c['id'] ?>" <?= $c['id']==$module['course_id'] ? 'selected':'' ?>>
            ...
            <div>
                ...
                <input type="text" name="duration" class="border p-2 w-full"
                       value="<?= htmlspecialchars($module['duration']) ?>">
            </div>

            <div class="flex justify-end gap-3">
                <a href="modules.php" class="px-4 py-2 border rounded">Cancel</a>
