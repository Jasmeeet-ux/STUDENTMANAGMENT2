...
<div class="mb-6">
    <div class="flex items-center space-x-3 mb-2">
        ...
        <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Assignment</h1>
    </div>
    <div class="text-sm text-slate-500 flex items-center ml-7">
        ...
        <i class="fas fa-chevron-right text-[10px] mx-2"></i>
        <span class="text-slate-700 font-medium"><?php echo htmlspecialchars($assignment['title']); ?></span>
    </div>
</div>

<form action="?module=assignments&action=update" method="POST" class="bg-white rounded-xl shadow-sm border border-bordercolor overflow-hidden max-w-4xl">
    ...
    <div class="px-6 py-4 border-b border-bordercolor bg-slate-50 flex items-center">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Update the assignment information.</p>
        </div>
    </div>
    ...
    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="md:col-span-2">
            ...
            <input type="text" name="title" value="<?php echo htmlspecialchars($assignment['title']); ?>" required class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
        </div>
        
        <div class="md:col-span-2">
            <label class="block text-sm font-semibold text-slate-700 mb-1">Description</label>
            <textarea name="description" rows="4" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary"><?php echo htmlspecialchars($assignment['description']); ?></textarea>
        </div>
        ...
        <div>
            ...
            <select name="class_id" required class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white">
                ...
                <?php foreach($classes as $c): ?>
                    <option value="<?php echo $c['id']; ?>" <?php echo $assignment['class_id'] == $c['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($c['course_code'] . ' - ' . $c['class_name']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div>
            ...
            <select name="subject_id" required class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white">
                ...
                <?php foreach($subjects as $s): ?>
                    <option value="<?php echo $s['id']; ?>" <?php echo $assignment['subject_id'] == $s['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($s['name'] . ' (' . $s['code'] . ')'); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div>
            ...
            <select name="teacher_id" required class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white">
                ...
                <?php foreach($teachers as $t): ?>
                    <option value="<?php echo $t['id']; ?>" <?php echo $assignment['teacher_id'] == $t['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($t['name']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div>
            ...
            <input type="datetime-local" name="due_date" value="<?php echo date('Y-m-d\TH:i', strtotime($assignment['due_date'])); ?>" required class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
        </div>
        
        <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Max Marks</label>
            <input type="number" name="max_marks" value="<?php echo floatval($assignment['max_marks']); ?>" step="0.01" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
        </div>
        ...
        <div>
            ...
            <select name="status" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white">
                ...
                <option value="Closed" <?php echo $assignment['status'] == 'Closed' ? 'selected' : ''; ?>>Closed</option>
            </select>
        </div>
    </div>

    <div class="px-6 py-4 bg-slate-50 border-t border-bordercolor flex justify-between items-center">
        <a href="?module=assignments&action=delete&id=<?php echo $assignment['id']; ?>" class="text-danger hover:text-red-800 text-sm font-medium transition-colors" onclick="return confirm('Are you sure you want to delete this assignment?');">
