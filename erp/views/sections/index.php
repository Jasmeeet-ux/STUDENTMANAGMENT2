...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Section Management</h1>
        <div class="text-sm text-slate-500 mt-1 flex items-center">
            ...
            <span class="text-slate-700 font-medium">Sections</span>
        </div>
    </div>
    <div class="flex space-x-3">
        <a href="?module=sections&action=create" class="bg-primary hover:bg-blue-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm flex items-center">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    <div class="bg-white p-4 rounded-xl shadow-sm border border-bordercolor flex items-center">
        ...
        <div>
            <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Total Sections</p>
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-bordercolor flex items-center">
        ...
        <div>
            <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Active Sections</p>
...
<div class="bg-white p-4 rounded-xl shadow-sm border border-bordercolor mb-6">
    <form method="GET" action="index.php" class="flex flex-wrap gap-4 items-end justify-between">
        <input type="hidden" name="module" value="sections">
        <div class="flex flex-wrap items-end gap-4 flex-1">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" placeholder="Section, Class..." class="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary w-64">
                </div>
            </div>
            <div>
                    ...
                    <tr>
                        <td colspan="7" class="p-12 text-center text-slate-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-base font-semibold text-slate-700">No sections found</p>
                                <a href="?module=sections&action=create" class="mt-4 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium shadow-sm hover:bg-blue-800">Add New Section</a>
                            </div>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($sections as $section): ?>
                        <tr class="hover:bg-slate-50 transition-colors group">
                            ...
                            <td class="p-4">
                                <a href="?module=sections&action=show&id=<?php echo $section['id']; ?>" class="font-bold text-primary hover:underline text-sm"><?php echo htmlspecialchars($section['name']); ?></a>
                                <div class="text-xs text-slate-500 mt-0.5"><i class="fas fa-door-open mr-1"></i> <?php echo htmlspecialchars($section['room_number'] ?? 'N/A'); ?></div>
                            </td>
                            <td class="p-4">
                                <p class="font-semibold text-slate-800 text-sm"><?php echo htmlspecialchars($section['class_name'] ?? 'Unassigned'); ?></p>
                                <p class="text-xs text-slate-500"><?php echo htmlspecialchars($section['course_name'] ?? ''); ?></p>
                            </td>
                            <td class="p-4">
                                <div class="flex items-center">
                                    <div class="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-xs font-bold text-slate-600 mr-2">
                                        <?php echo substr($section['teacher_name'] ?? 'U', 0, 1); ?>
                                    </div>
                                    <span class="text-sm font-medium text-slate-700"><?php echo htmlspecialchars($section['teacher_name'] ?? 'Not Assigned'); ?></span>
                                </div>
                            </td>
                            ...
                            <td class="p-4 text-right">
                                <div class="flex items-center justify-end space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href="?module=sections&action=show&id=<?php echo $section['id']; ?>" class="text-slate-400 hover:text-primary transition-colors" title="Manage Section">
                                        <i class="fas fa-cog"></i>
                                    </a>
                                    <a href="?module=sections&action=edit&id=<?php echo $section['id']; ?>" class="text-slate-400 hover:text-accent transition-colors" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <a href="?module=sections&action=delete&id=<?php echo $section['id']; ?>" class="text-slate-400 hover:text-danger transition-colors" title="Delete" onclick="return confirm('Are you sure you want to delete this section?');">
