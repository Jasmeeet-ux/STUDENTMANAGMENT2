...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Subject Management</h1>
    ...
    <div class="flex space-x-3">
        <a href="?module=subjects&action=create" class="bg-primary hover:bg-blue-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm flex items-center">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    <div class="bg-white p-4 rounded-xl shadow-sm border border-bordercolor flex items-center">
        ...
        <div>
            <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Total Subjects</p>
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-bordercolor flex items-center">
        ...
        <div>
            <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Active Subjects</p>
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-bordercolor flex items-center">
        ...
        <div>
            <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Total Credits</p>
            <h3 class="text-2xl font-bold text-slate-800"><?php echo $stats['total_credits'] ?? 0; ?></h3>
        </div>
    </div>
</div>
        ...
        <div class="flex flex-wrap items-end gap-4 flex-1">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" placeholder="Subject Name, Code..." class="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary w-64">
                </div>
            </div>
            <div>
                    ...
                    <tr>
                        <td colspan="8" class="p-12 text-center text-slate-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-base font-semibold text-slate-700">No subjects found</p>
                                <a href="?module=subjects&action=create" class="mt-4 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium shadow-sm hover:bg-blue-800">Add New Subject</a>
                            </div>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($subjects as $subject): ?>
                        <tr class="hover:bg-slate-50 transition-colors group">
                            ...
                            <td class="p-4">
                                <span class="font-bold text-slate-800 text-sm"><?php echo htmlspecialchars($subject['code']); ?></span>
                            </td>
                            <td class="p-4">
                                <p class="font-semibold text-slate-800 text-sm"><?php echo htmlspecialchars($subject['name']); ?></p>
                            </td>
                            <td class="p-4">
                                <div class="font-semibold text-slate-800 text-sm"><?php echo htmlspecialchars($subject['course_name'] ?? 'Unassigned'); ?></div>
                                <div class="text-xs text-slate-500"><?php echo htmlspecialchars($subject['course_code'] ?? ''); ?></div>
                            </td>
                            <td class="p-4">
                                <p class="text-sm text-slate-700"><?php echo htmlspecialchars($subject['teacher_name'] ?? 'Not Assigned'); ?></p>
                            </td>
                            <td class="p-4 text-center">
                                <span class="font-bold text-primary"><?php echo htmlspecialchars($subject['credits']); ?></span>
                            </td>
                            ...
                            <td class="p-4 text-right">
                                <div class="flex items-center justify-end space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a href="?module=subjects&action=edit&id=<?php echo $subject['id']; ?>" class="text-slate-400 hover:text-accent transition-colors" title="Edit">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <a href="?module=subjects&action=delete&id=<?php echo $subject['id']; ?>" class="text-slate-400 hover:text-danger transition-colors" title="Delete" onclick="return confirm('Are you sure you want to delete this subject?');">
